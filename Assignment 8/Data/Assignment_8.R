...
# INTERPRETATION: There are a good amount of missing values across the dataset, requiring imputation.
...
# Using median (rather than mean) for continuous variables is preferred because it is robust
# to extreme values and skewed distributions, which are common in wine chemistry data.
#
# RECOMMENDATION: Before imputing, examine whether missingness is random (MCAR/MAR) or
# systematic (MNAR). If a variable has >30% missing values, consider dropping it entirely
# rather than imputing, as high imputation rates can distort the variable's distribution
# and introduce bias into downstream models. Consider adding binary "missing indicator"
# flag columns for heavily imputed variables so the model can learn from the pattern
# of missingness itself.
...
# INTERPRETATION: Outliers are present across multiple continuous wine chemistry variables.
# Variables such as ResidualSugar, Chlorides, FreeSulfurDioxide, and TotalSulfurDioxide
# tend to exhibit the most extreme outliers due to their naturally right-skewed distributions.
# These extreme values are likely legitimate measurements from atypical wines rather than
# data entry errors, meaning they carry real information but may unduly influence model
# coefficient estimates if left uncorrected.
#
# RECOMMENDATION: Rather than removing outlier rows (which reduces sample size and may
# introduce selection bias), apply winsorization to cap extreme values at the IQR fence
# boundaries. This preserves all observations while limiting the leverage of extreme values.
# For any variable where outliers exceed ~10% of observations, revisit its distribution
# and consider whether a log or square-root transformation might be more appropriate
# than winsorization prior to modeling.
...
## INTERPRETATION: Winsorization was successfully applied to all continuous variables,
## capping values at the 1.5×IQR lower and upper boundaries. All original rows are
## retained (100% row retention), so no information is lost from the dataset.
## The capped values reduce the influence of extreme observations on the model's
## coefficient estimates without artificially shrinking the sample size.
## Variables like ResidualSugar, Chlorides, and the sulfur dioxide variables likely
## had the greatest number of values capped, given their known right-skewed distributions
## in wine chemistry data.
##
## RECOMMENDATION: Confirm that the winsorized distributions are more symmetric by
## re-examining histograms before modeling. If a variable still shows heavy skew after
## winsorization, apply a log or square-root transformation as an additional step.
## Document the exact upper and lower bounds applied per variable so results can be
## reproduced on any new scoring data.
...
# of patrons (approximately 78.6%) purchasing a case of wine. This imbalance means a
# naive model that always predicts "Purchased" would already achieve ~78.6% accuracy,
# so overall accuracy alone is a misleading performance metric. The minority class
# (non-purchase, ~21.4%) is the more informative and difficult class to predict correctly.
#
# RECOMMENDATION: When evaluating model performance, prioritize metrics that account
# for class imbalance — specifically sensitivity (recall for the minority class),
# specificity, and the AUC-ROC rather than raw accuracy. If sensitivity on the
# non-purchase class is critically important (e.g., to identify at-risk customers),
# consider resampling techniques such as SMOTE or adjusting the classification threshold
# below 0.5 to improve detection of the minority class.
